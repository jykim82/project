...
## 항목 직접 입력 — 개별 기재 (2026-07-22)

**취지 재확인 (사용자 지시):** 일 점검 보고서는 알람·이력에서 발췌해 만드는
것이 아니라 **하루 일상 점검을 개별 사용자가 직접 입력해 추가하는 것이 기본**
이다. AI 채팅 등록 경로는 "현장에서 추가해두고 센터에서 문구를 수정·보완"하는
보조 흐름. **장애 조치 보고서도 동일하게** 이력 없이 직접 기재 가능해야 한다.

구현:
- `POST /reports/{id}/items/manual` — task_id NULL 수동 항목 (0058 스키마의
  "NULL = 수동 입력, P2" 실현). 내용(occurred_text) 필수, AI 요약 없음.
  시설유형/현장명/설비명/분류·점검종류/일시/조치내용 선택 입력.
- 프런트 `ManualItemDialog` — 보고서 상세의 **"항목 직접 입력"** 버튼
  (draft + 작성자만). 일 점검은 점검종류(일상/정기/특별)·점검 내용 라벨,
  장애 조치는 분류(고장/이상/교체/점검)·발생 내용 라벨.
- **빈 보고서 생성 허용** — 추가 다이얼로그에서 이력 0건 선택 시
  "빈 보고서 생성" (두 유형 공통). 생성 후 상세에서 개별 기재.
- 수동 항목도 기존 항목 편집/AI 정제/사진 추가/인쇄 흐름 그대로 적용.

---

## 부기 (2026-07-28) — 보고서 제목 인라인 편집

보고서 상세 표지의 제목은 담당자가 직접 수정할 수 있다 (연필 → input →
저장/Enter, Esc 취소). 같은 날짜라도 "네트워크 장애 조치"/"펌프 장애 조치"
처럼 내용이 달라 자동 제목만으로는 부족하다는 사용자 요청. **확정 후에는
잠금** (공문 확정본 보호). 백엔드 `PATCH /reports/{id}` 의 title 은 기존
지원 — UI 만 부재였다.
