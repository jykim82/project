...
## 계기 지시값 관찰 한 줄 (2026-07-16 — 라이트 통합)

진단 사진에 게이지·인디케이터가 판독 가능하게 보이면 `observed_state` 에
관찰 한 줄 추가 (vision_agent `_DIAGNOSE_PROMPT_TEMPLATE` §계기 관찰 가이드):
- 단위가 계기 면에 보일 때만 단위 포함 — **단위·배율 추측 금지** (Zero-Hallucination)
- 안 보이면 "(단위는 계기 명판 확인 필요)", 불명확하면 "명확히 판독되지 않습니다"
- 프런트 무변경 (VisionAdviceCard 가 observed_state 렌더)
- **본격 기능화(계기 메타·SCADA 대조·점검 기입)는 보류** — 사용자 결정
  "숫자를 보고서에 기록할 일은 없다". PoC 능력 확인: 합성 게이지 5/5
  (아날로그 오차 ≤2.7%, 디지털 완벽). 고객 계기 검침 요구 발생 시 재개
  (docs/review-items.md 참조)
