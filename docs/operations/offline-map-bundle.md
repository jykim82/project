...
## 공공 납품 시 배경지도 — VWorld/국가공간정보 경로 (2026-08-03)

국가기관·지자체 납품은 국가 공간정보가 **무상 제공 대상**이라, 기본 OSM
번들 대신 발주처 데이터로 배경지도를 구성할 수 있다.

**원칙 — 데이터 권한 주체는 발주처**: "우리 제품에 VWorld 를 담아 판매"가
아니라 "발주처 보유 데이터를 발주처 시스템에 구축"이어야 한다. VWorld
약관(타일 대량 다운로드·저장 후 재제공 금지)의 책임 구조를 피하는 유일한
정당 경로. 과업지시서·계약서에 **"관할 공간정보 데이터는 발주처가 제공"**
문구를 넣을 것.

절차:
1. 사업 착수 시 발주처 명의로 관할 데이터 확보 (브이월드 공공 인증키
   또는 국가공간정보센터 제공 — 배경지도·정사영상·수치지형도)
2. QGIS 로 가공(업계 관행 도구 그대로): 레이어 구성 후
   Processing → "XYZ 타일 생성 (MBTiles)" 로 관할 bbox 내보내기
3. `pmtiles convert region.mbtiles region.pmtiles`
4. 관리 UI 에서 베이스맵 업로드 교체 (`files/map/` — 즉시 반영, §업로드)

- 제품 기본 번들은 OSM(ODbL) 유지 — 개발·데모·데이터 미확보 폴백
- 래스터 타일(MBTiles→pmtiles)은 벡터보다 용량이 큼 (줌 상한을 관할
  규모에 맞게 — z16~17 권장, 항공사진은 별도 판단)
- 출처 표기: OSM 은 "© OpenStreetMap contributors, Protomaps", 발주처
  데이터는 해당 기관 표기 규정 — 납품 문서에 명기

### 계획 — VWorld 타일 수집·변환 독립 프로그램 (보류, 요청 시 착수)

> 2026-08-03 사용자 지시 — **계획만 등재, VWorld 진행 재요청 시 착수.**

구축 담당자가 QGIS 없이 쓰는 **별도 변환 프로그램** (CLI, 폐쇄망 반입형):
- 입력: VWorld API 키 + 관할 bbox + 줌 상한(권장 z16)
- 동작: XYZ 타일 순회 다운로드 → **일 호출 한도 관리 + 중단 이어받기**
  → MBTiles 패킹 → `pmtiles convert` → `region.pmtiles` 출력
- 함께 필요한 제품 수정: GisMap **래스터 pmtiles 소스 지원** (현 스타일은
  벡터 전제. 다크모드는 밝기 조정 대응, 라벨은 벡터 오버레이 하이브리드
  검토)
- 성능 참고: 래스터는 Range 조각 읽기라 운영 속도 저하 없음 — 용량은
  번들 반입 시간에만 영향 (z16 기준 100~200MB급)
- 전제: 데이터 권한은 발주처 명의 (위 "공공 납품 시 배경지도" 절차)
