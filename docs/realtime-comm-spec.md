...
- **그룹 채팅 (2026-07-20 확장, Migration 0113)**: room_id `grp:<id>` +
  tb_chat_group(제목 — 미지정 시 참가자 이름 자동)/tb_chat_group_member.
  메시지·읽음·unread·첨부·도착 토스트는 기존 tb_user_chat_* 그대로 재사용.
  "새 대화"에서 **1명 = DM / 2명 이상 = 그룹 생성**(+그룹 이름 입력).
  멤버십 서버 검증(비멤버 403). 목록 아이콘: 그룹=👥 보라.
  **v2 (2026-07-21 구현)**: 그룹 스레드 헤더 ⚙ → 멤버 목록·추가 초대·
  이름 변경(멤버 누구나)·나가기. 메신저 헤더 🕘 통화 이력
  (`GET /call/history` — 발신/수신·유형·결과·통화시간, 페이징).
...
- **v1 구현 완료 (2026-07-20, Migration 0112)** — 풀메시 3~4인 음성 회의:
  - 흐름: 메신저 헤더 회의 버튼 → 참가자 선택(최대 3명) → 초대 벨
    ("N인 회의에 초대") → 참여한 사람부터 쌍별 P2P 연결
  - DB: tb_conf_call(방)·tb_conf_member(invited→joined/rejected/missed→left)·
    tb_conf_signal(쌍별 SDP 사서함, 1회성 소비)
  - 프로토콜: 신규 합류자가 기존 joined 전원에게 offer, 기존측 answer.
    상태 폴링 2.5s(=하트비트, 30s 끊김 left), 시그널 2s, 벨 20s missed,
    joined ≤1 시 회의 자동 종료. 마이크 1개 공유, 원격은 피어별 audio 믹싱
  - UI: 참가자 바 (이름 + 상태 점: 초록=연결/노랑=대기/회색=이탈,
    전체 음소거·나가기). TURN 옵션 그대로 적용 (외부망 참가 가능 —
    릴레이 대역폭 N배 유의)
  - **v2 (2026-07-21)**: 통화 중 추가 초대 — 참가자 바 ➕ (joined 만 가능,
    총원 상한 4, 거절/부재 멤버 재초대 지원). 잔여: 영상 회의, 개별 음소거
- 5인 이상 정례 회의가 요구되면 SFU 도입 검토 (폐쇄망 자체 호스팅 가능).
