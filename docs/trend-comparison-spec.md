...
시간대 평균 모양은 안정적이다 — 수위와 달리 **사람 활동이 결정하는 신호**라
패턴 투영 기각 사유가 적용되지 않는다. `_flow_hourly_profile`:
- 계절 나이브 미달 시 `trend_kind=flow` 한정 — **14일 시간대 프로파일 모양
  + 최근 24h 수준 보정** 투영 (`method: hourly_profile`), Chronos 미호출
- **전수 백테스트 (순시유량 유신호 81태그 × 7일)**: 수평 대비 74/81 우세 ·
  개선율 중앙값 +27%. 레벨 보정이 프로파일 원판의 악화 태그(행정2-3 등
  레벨 이동 케이스)까지 해소
- **게이트: split-half r>=0.3** (전반 7일 vs 후반 7일 프로파일 상관) —
  시간대 모양이 없는 공업 유량(실측 석문공업 -0.13·송악1공업 0.08)에
  가짜 일주기를 입히지 않고 Chronos 폴백
- 스파이크는 예측하지 않는다 — 밴드(시간대 ±1.28σ ≈ 10~90%)가 일간 변동
  폭을 정직하게 표시. 연속성 앵커(초반 2h 램프)·관측 밴드 클램프 동일 적용
