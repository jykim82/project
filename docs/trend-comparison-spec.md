...
**야간최소유량 시계열 비교 스킵 [E-061] (2026-09-01):** FACILITY_TREND
야간최소유량 fast-path 는 행을 일별 NMF 집계로 교체하므로 범용 비교가
원시 시간평균 기준선과 물리량 불일치 — `_nmf_series` 턴 파생 플래그로
비교를 스킵한다 (배지 없음). NMF 이상 판정은 누수 CUSUM(leak-alert-spec)
전담. 원칙: 같은 신호에 판정 체계가 둘이면 화면에는 전담 하나만.
...
클램프 후에도 `hours_to_threshold`("N시간 후 한계 접근")는 기울기로 계산되어
의미를 유지한다.

**진폭 압축 "참고" 톤 (2026-07-29, 합덕인더스 생활수위 실사):** 전망 범위가
관측 진동의 40% 미만이면 `low_confidence` — 펌프 주기가 불규칙한 수위
(ACF 0.13)는 Chronos 중앙 경로가 개별 진동을 못 그리는데, CV 게이트(>15%)
는 평균이 큰 수위 신호(CV 5.6%·진동 0.87m)를 놓쳐 확정 톤으로 표시되던
공백. 유의 진동(관측 범위 > 수준의 10%)일 때만 적용 — 평탄 신호의 평탄
전망은 정상. 진동 정보는 불확실성 밴드가 담당한다.

**전망 렌더 재보간 [E-059] (2026-07-29):** x축이 category(포인트당 등폭)라
관측 버킷(예: 5분)과 전망 간격(30분)이 다르면 전망 구간이 압축/과신장돼
파형이 왜곡된다. `comparison-overlay.ts` `resampleForecastToBucket` 가
전망 시리즈·밴드를 **관측 버킷 간격으로 선형 재보간**해 연결 — 수치 불변,
표시만 시간 비례 (매방리 압력 실측: 전망 폭 2.3% → 9.2% ≈ 창의 1/8).

**수위 하한(저수위·LL) 판정 (2026-07-29):** 수위는 상한(넘침·HH)뿐 아니라
하한(공급량 부족·LL)도 위험이다. `_lookup_level_lower` 가 SCADA 실측 임계
(LEI↔LEC 페어, `datainfo LL` — alarm_approach 와 동일 명명 규칙·41페어)를
조회해 후보에 추가하고, 교차 스캔이 **먼저 도달하는 쪽**으로 판정한다:
- 응답 계약은 단일 `threshold_value/label` 유지 (프런트 무변경) — 실제
  교차하는 쪽, 교차 없으면 **전망 기울기가 향하는 쪽** 임계를 내려준다.
  하강 중인 배수지에 만수위 선만 보여주던 공백이 이것.
- **방향 명시**: HH 는 상향 교차만·LL 은 하향 교차만 위험. 이미 LL 미달인
  현장(실측: 난지관광 0.11<0.5)에서 회복 상승을 "도달"로 오판하지 않는다.
- 배지 문구에 방향 표기 — "N시간 후 **저수위/만수위** 한계 도달".
- LL 페어 없는 현장은 하한 후보 없음 — 임의 하한을 만들어내지 않는다.
- 하한 근사를 쓰지 않는 이유: 상한은 만수위(탱크 제원)로 근사 가능하지만
  하한은 물리 기준이 없다(운영 판단값). 실측 임계만 신뢰.

**향후 전망 R² 게이트 (약한 추세=수평 / 강한 추세만 투영) (2026-07-11):** 순수 선형
외삽은 '지금 오실레이션의 어느 위상인가'를 그대로 연장해, 진동 신호(배수지 수위)에서
순간 하강 구간을 잡으면 밴드를 벗어나 급락한다(관측 2.97~3.41 인데 전망은 2.1). 회귀
직선은 **정의상 진동 불가**이므로, 회귀 **적합도 R²** 로 추세 투영 여부를 게이팅한다
(`_linear_forecast`, 마지막 실측값 `last_y` 기준):
```
trend_w = clamp((R² - 0.4) / 0.4, 0, 1)               # R²≤0.4→0, R²≥0.8→1 램프
forecast[i] = last_y + (slope·trend_w)·dh_i           # 추세 성분(강한 추세만)
            + (mean - last_y)·(1-φ^i)·(1-trend_w)      # 최근 평균 회귀(약한 추세=수평), φ=0.90
```
- **약한/애매한 추세(진동, R²↓):** 추세 성분 0 → 최근 평균으로 **수평 수렴**. "직선으로
  계속 떨어지는" 현상 제거(현 수준 유지 전망).
- **뚜렷한 추세(R²↑):** 방향 그대로 투영(물리 클램프 별도).
- 임계 도달 시점은 forecast 시리즈에서 첫 교차 직접 스캔(선형 slope 아님).
- **시도했으나 채택 안 함 — 패턴 투영 (수위):** baseline(시간대 기대값)을 미래로 투영해
  '진동하는 예측'을 만들려 했으나, 배수지 수위는 **운영(펌프 on/off)으로 결정**돼
  시간대 주기 패턴이 신뢰 불가(학습창의 과거 저수위가 패턴 오염 → 전망이 1.3 까지
  급락). 진동을 신뢰성 있게 예측할 수 없으므로 '약한 추세=수평'이 최선.
  **단, 이 기각 사유는 수위 한정** — 유량은 아래와 같이 패턴 투영을 채택.

**유량 시간대 프로파일 투영 (2026-07-29, 남산1 괴리 보고에서 출발):** 소비
유량은 스파이크 시각이 매일 달라 30분 격자 ACF 가 낮지만(남산1 실측
r@24h=0.044 → 계절 게이트 미달 → Chronos 중앙값 진폭 압축·평평한 전망),
시간대 모양은 안정적이다 — 수위와 달리 **사람 활동이 결정하는 신호**라
패턴 투영 기각 사유가 적용되지 않는다. `_flow_hourly_profile`:
- 계절 나이브 미달 시 `trend_kind=flow` 한정 — **14일 시간대 중앙값
  프로파일** 투영 (`method: hourly_profile`), Chronos 미호출
- **평균이 아니라 중앙값** (같은 날 신평 유입 실사로 확정): 배수지 유입처럼
  켜짐(~300)/꺼짐(0) **이봉 신호에서 평균은 실재하지 않는 값**(새벽 110)을
  그린다 — 실측은 0 을 찍는데 전망만 안 내려가는 괴리. 중앙값은 다수
  상태를 따라 실측처럼 0 까지 내려가고 MAE 도 우수
- **전수 백테스트 (순시유량 유신호 81태그 × 7일)**: 중앙값 프로파일이
  수평 대비 **76/81 우세 · 개선율 중앙값 +29%** (평균+레벨보정 74/81·+27%
  대비도 우세 — 신평 유입 55.4→38.9, 남산1 21.9→18.0). 레벨 보정은 이봉
  꺼짐 구간까지 밀어올려 손해라 **미적용** — 이음새는 2h 램프가 담당
- **게이트: split-half r>=0.3** (전반 7일 vs 후반 7일 프로파일 상관) —
  시간대 모양이 없는 공업 유량(실측 석문공업 -0.13·송악1공업 0.08)에
  가짜 일주기를 입히지 않고 Chronos 폴백
- 스파이크는 예측하지 않는다 — 밴드 = **시간대 p10~p90 분위수**: 이봉
  구간에선 0~300 전폭으로 벌어져 "이 시간대는 켜짐/꺼짐 둘 다 있음"을
  정직하게 표시. 연속성 앵커(초반 2h 램프)·관측 밴드 클램프 동일 적용

**검증 방법(권장):** forecast 변동 범위를 **같은 표출 기간(24h) 실측 변동(min~max)**과
비교 — forecast 가 실측 밴드를 벗어나면 비현실적. 검증: A구설1지 실측 2.97~3.41 →
전망 3.12~3.14(수평, 밴드 내). 신설5지 실측 2.78~3.52 → 전망 3.17~3.70(실 상승 반영).
수정 전 2.1(실측 최소 2.97 보다 0.87m 아래) 대비 정상화.
...
- 프런트 밴드 렌더 완료(2026-07-16): comparison-overlay 공용 빌더에 stack
  기법 음영 — 채팅/트렌드 페이지/GIS 팝업 공통. 3회 UI 테스트 + 스모크 2종 통과.
...
**③ GBT 예측 시(hour) 버킷 최적화 (완료 2026-07-14):**
- **원인:** `gbt_baseline()` 이 target_time **타임스탬프마다** 피처 행 생성 + `_roll()`
  이 매번 hourly dict 전체 스캔 (7일 1분 버킷 = 10,080행 × 2회 × ~338시간 ≈ 태그당
  680만 회 연산). 4태그 벤치 78.1s.
- **통찰:** 모든 피처(캘린더·lag·rolling)는 **시(hour) 단위에서만 변한다** —
  `_at`/`_roll` 은 `floor('h')` 기반이고 KST 는 고정 오프셋(+9)이라 같은 UTC 시각 내
  타임스탬프는 피처가 완전히 동일. 따라서 근사가 아니라 **완전 동치 변환**.
- **해결:** 고유 시 버킷(7일 ≈ 169행)만 피처 계산·예측 후 `hour_keys` 로 타임스탬프에
  매핑 (`trend_baseline.py gbt_baseline`).
- **검증:** ① 고정 시간창 동일성 — 구/신 구현 10,080개 값 **완전 일치(불일치 0건)**.
  ② 벤치 78.1s → 3.06s (25×, 태그당 19.5s → 0.76s). ③ 실 UI 트렌드 페이지 4태그
  7일 /trend/data **636ms**, 평소 대비·향후 전망 배지 정상.
- 주의: 첫 요청이 느리면 uvicorn 재기동 콜드스타트(스캔 캐시·DB 풀 초기화)이지 GBT
  아님 — 웜 상태로 재측정할 것.

---

> **전망 참고 톤 (2026-07-25):** ACF 지배 주기 미검출 + 변동계수 CV>15%
> 시설은 `forecast.low_confidence=true` — 배지에 "참고 ·" 접두 + 사유 툴팁
> (펌프 기동정지형 등 원리적으로 예측 어려운 신호의 확정 톤 방지).
...
## 12. 향후 전망 v2 (2026-07-23 — E-048~E-050 반영)

사용자 5회 검증(급락→주기 왜곡→불연속→파형 상이→고점 초과)으로 확정된 현행 설계:

1. **엔진 우선순위**: `seasonal_{N}h`(지배 주기 재현) → `chronos_bolt` → `linear`
2. **시간 격자 컨텍스트**: 관측 버킷을 균일 30분 격자에 시간 기준 배치
   (결측 보존 — 위치 기반 압축은 시간축이 줄어 주기 검출이 깨짐)
3. **지배 주기 탐지**: 30분 격자 ACF 스캔 8~36h, 최대 r ≥ 0.6 이면 직전
   주기 패턴 반복 (난지마을 = 14h r 0.81. 고정 24h 검사는 역위상 오판)
4. **연속성 앵커**: 전망 시작 = 마지막 실측. 오프셋은 **초반 2h 램프**에서만
   소멸 (전 구간 점감은 패턴 고점을 관측 최대 위로 들어 올림)
5. **Chronos 호출 규약**: 예측 스텝의 시간 단위 = 입력 간격. 30분 격자
   컨텍스트로 48스텝(원생 한도 64 이내) **원샷** — 세분 예측 후 추출 금지
   (자기회귀 이어붙기로 주기 붕괴). Chronos 중앙값은 위상 불확실 신호에서
   진폭이 눌리며 이는 정직한 표현 — 진폭은 10~90% 밴드가 담당
6. **클램프**: 관측 밴드 + 마진. 수위 만수위 캡(threshold/0.9)은 threshold 가
   관측 최대보다 **위일 때만** (하한 임계에 적용하면 고점 절단)
7. **method 라벨**: `seasonal_{N}h` → "반복 주기 패턴 (N시간 주기 재현)"
   (comparison-overlay 동적 매핑)
8. **검증 기준 4종**: 진폭 · 주기 · 연결점(격차 0) · 범위(관측 max 이하)
9. 운영 전제: Chronos 는 컨테이너 이미지에 torch/chronos-forecasting 필요
   (E-048 — 재생성 시 이미지 재빌드로 반영)

## 13. 적산계 태그 적산차 표시 (2026-07-22~23)

- `/trend/data`: datainfo~'적산' 태그를 버킷 max(단조 증가라 = 마지막 값)
  인접 차분으로 변환 (음수=리셋/롤오버 → 결측). `accum_diff_tags` 응답,
  트렌드 메뉴 시리즈명 "(적산차)" 표기 (displayTags)
