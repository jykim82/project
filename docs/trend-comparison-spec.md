...
**향후 전망 R² 신뢰도 감쇠 (2026-07-11):** 클램프만으로는 부족 — 진동/평균회귀
신호(배수지 수위 등)에서 최근 구간의 하강 기울기를 그대로 24h 연장하면 밴드를
벗어나 급락하는 비현실적 직선이 나온다(관측 2.85~3.5 인데 전망은 2.1 로 급락).
`_linear_forecast` 에서 회귀 **적합도 R²** 를 계산해 기울기를 감쇠한다:
- `slope_eff = slope × R²` — 진동(낮은 R²)은 평탄 외삽에 수렴, 뚜렷한 추세(높은
  R²)는 거의 그대로 반영.
- 외삽 기준점을 **마지막 실측값**으로 잡아 실측선과 이음새를 자연스럽게 연결
  (회귀선 절편 기준 → 마지막 실측값 기준).
- 반환 slope 도 `slope_eff` 라 `_hours_to_threshold`·status 판정이 감쇠 기울기와
  일관. 검증: 행정 배수지 수위 forecast 3.0→2.1 급락 → 3.24~3.29 밴드 내 평탄.
