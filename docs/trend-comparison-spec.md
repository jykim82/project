...
**향후 전망 R² 블렌딩 (추세 외삽 vs 최근 평균 회귀) (2026-07-11):** 클램프만으로는
부족 — 순수 선형 외삽은 '지금 오실레이션의 어느 위상인가'를 그대로 연장하므로,
진동 신호(배수지 수위)에서 순간 하강/상승 구간을 잡으면 밴드를 벗어나는 급락/급등
직선이 나온다(관측 2.85~3.5 인데 전망은 2.1 로 급락). `_linear_forecast` 에서 회귀
**적합도 R²** 로 두 성분을 블렌딩한다(마지막 실측값 `last_y` 기준):
```
forecast[i] = last_y + (slope·R²)·dh_i               # 추세 성분 (신뢰 높을수록)
            + (1-R²)·(mean - last_y)·(1-φ^i)          # 평균회귀 성분 (진동 신호), φ=0.90
```
- **진동(R²↓):** 최근 평균(mean)으로 평탄 수렴 → 위상 방향 따라가지 않음.
- **뚜렷한 추세(R²↑):** 방향 그대로 외삽(물리 클램프 별도 적용).
- **임계 도달 시점(`hours_to_threshold`):** 블렌딩 곡선이 평균회귀로 평탄해지므로
  선형 slope 로 계산하면 어긋난다 → **forecast 시리즈에서 첫 교차를 직접 스캔**해
  곡선과 일관되게 산출.
- 검증: A구설1지(진동) 3.0→2.1 급락 → 3.13→2.80 밴드 내 완만. 신설5지(실 상승)
  4.20(만수위) → 3.90 완화, 한계 접근 12h→20h.
