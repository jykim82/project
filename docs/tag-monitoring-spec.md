...
- **L1→L3 역추적 위치 (구현됨, v1.6):** ping 진단과 측정 태그의 연결은
  태그 모니터링이 아닌 **네트워크 화면(`/network`) 장비 상세 패널**에서 제공한다.
  장비(토폴로지 노드) 선택 시 `GET /network/devices/{equipment_id}/tags` 로
  해당 장비 뒤 측정 태그(L3) + 현재값을 조회해 "영향 측정 태그" 섹션에 표시.
  ping 죽은 `plc_37` 선택 시 신평2 유량·압력 태그가 신선 값으로 보여
  "ping 장애 ≠ 데이터 손실"을 운영자가 직접 확인 (`AffectedTagsSection`).
  진입 동선: 토폴로지 그래프 노드 클릭 **또는** 하단 장비 상태 테이블 행 클릭
  (선택 행 하이라이트 + 토폴로지 카드로 스크롤).
...
- 2026-06-13 v1.6 — L1→L3 역추적을 네트워크 화면에 구현(§3.0.1). 신규
