...
- **P2 (구현 완료 2026-07-25): CSV 신규 태그 온보딩** — `/onboard/preview`
  (일괄 변환 + new/exists/invalid/dup 상태 + **조회 계약 lint 4종**: 유량
  키워드·적산 표기·미변환 약어·DI 상태 어휘) → `/onboard/apply`(기존 tagsn
  보호 스킵·이력 로그). 화면: CSV 템플릿·업로드·경고 테이블·일괄 등록 카드.
  신규 고객 태그 대량 등록의 표준 경로 — datainfo 는 자동 생성
