...
## 구축 워크스페이스 캔버스 정렬 ✅ 해결 (2026-09-19 구현 완료)
...
**조치:** `align-utils.ts`(순수 계산) + `AlignButtons.tsx`(공용 툴바 조각).
양 캔버스에 `selectionOnDrag + panOnDrag=[1,2]`(좌=박스 선택, 우/휠=팬).
계통도는 store `alignSelected`(undo 히스토리 통합·isDirty), 통신망은 변경분
즉시 `canvas-positions` 저장. 등간격은 양 끝 고정 · 사이 간격 균등(3개↑).
Playwright 실검증: 박스 선택 6/24개·정렬 수렴·undo 복원·우클릭 팬·저장 PUT.
