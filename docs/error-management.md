...
- **추가 (사용자 4차 검증 "실측은 3 이하인데 예측이 3 초과"):** 연속성 앵커 오프셋을 전 구간 점감시키면 패턴 고점이 관측 최대 위로 들림 → **초반 램프(~2h)에서만 소멸**로 변경. 검증: 실측 max 3.01 / 전망 max 2.99, 연속 격차 0.0
...
### [E-051] 이상 진단 "평소 대비 +1257%" — 기준선이 계측 개시 이전 노이즈로 오염

- **날짜:** 2026-07-23
- **증상:** 합덕일반(배) 공업 수위2 — 실측 2.3~2.55로 안정인데 "값이상 (+1257.1%, 가동률 43%)" 판정
- **원인:** 태그가 7/13에야 실계측 시작 — 이전 수개월은 0/노이즈(0.01대). 기준선 SQL의 절대 필터(>0.001)가 노이즈를 "가동 데이터"로 통과시켜 평균이 0.18로 오염 (365일 창·cagg 이력 지배)
- **해결:** 공유 기준선 SQL(이상 진단·ANOMALY_SCAN_ALL)에 `tag_scale` CTE — **최근 3일 중앙값×10%** 를 유효 하한으로(현재 운영 레짐 앵커, LEFT JOIN+COALESCE 폴백 0.001). P95×5% 1차안은 노이즈가 분포 95%를 차지해 무력화됨. 가동률(active_pct)은 기존 절대 기준 유지. 검증: 평균 0.18→2.35, z 3.63→0.19, 편차 +1257%→+2.6%
- **재발 방지:** ① 신규 계측 태그(레짐 체인지)는 절대 임계 기준선이 항상 왜곡 — 기준선 필터는 **현재 레짐 상대 기준**으로 ② "편차 %가 수백~수천"이면 값이 아니라 기준선 오염 먼저 의심 ③ ANOMALY_COMPARE 는 구조가 달라 미적용 — 동일 증상 발견 시 같은 패턴 적용 (review-items)
- **✅ 마감 (2026-07-24):** ANOMALY_COMPARE 에도 tag_scale 적용 완료 — E-051 계열 전 경로(이상 진단·SCAN_ALL·COMPARE) 일원화. 스모크 16/16

### [E-052] 네트워크 링크 폼 프로토콜 항상 빈 목록 — tb_protocol_lookup 부재

- **날짜:** 2026-07-26
- **증상:** 네트워크 연결 추가 폼(표 탭·링크 에디터 공통)의 프로토콜 선택이 항상 비어 있어 필수값을 채울 수 없음 — UI 로는 링크 등록이 원천 불가 (기존 280링크는 전부 CSV 업로드 경로)
- **원인:** `GET /network/links/protocols` 가 참조하는 `tb_protocol_lookup` 테이블이 생성된 적 없음 — API·폼 개발 시 마스터 시드가 누락된 채 CSV 경로만 검증됨
- **해결:** Migration 0124 — 테이블 생성 + 실사용 프로토콜 5종(tb_network_link DISTINCT) + 표준 3종 시드. 링크 에디터 E2E 에서 드래그 연결→폼 프리필→프로토콜 선택 가능 확인
- **재발 방지:** ① 폼 필수값이 마스터 테이블 의존이면 **마스터 시드를 같은 마이그레이션에 포함** ② 신규 폼 검증은 CSV 우회 경로가 아니라 폼 자체로 1회 저장까지 ③ 룩업 테이블 부재는 500이 아니라 "빈 목록"으로 조용히 실패 — 필수 select 가 비면 룩업 원본부터 확인

### [E-053] 링크 에디터에서 통신 링크가 클릭으로 선택되지 않음 — onEdgesChange 미연결

- **날짜:** 2026-07-26
- **증상:** 네트워크 링크 에디터에서 링크(선)를 정확히 클릭해도 선택되지 않음 → 선택 기반 수정·삭제가 전부 동작 불가. 노드(장비) 클릭은 정상 선택
- **원인:** React Flow 는 **선택 상태도 변경(change) 이벤트로 전달**한다. 노드는 `onNodesChange`+`applyNodeChanges` 가 연결돼 있어 선택이 반영됐지만, 엣지는 `onEdgesChange` 를 넘기지 않아 `addSelectedEdges` 가 만든 select change 가 버려짐. 기존에는 "엣지 클릭 = 즉시 삭제 확인창"이라 선택 불가가 드러나지 않고 가려져 있었음
- **해결:** `onEdgesChange={(c) => setEdges((e) => applyEdgeChanges(c, e))}` 연결. 실클릭 E2E 로 `.react-flow__edge.selected` + 툴바 "선택: 링크 …" + 엣지 삭제 버튼 노출 확인
- **재발 방지:** ① React Flow 를 controlled 로 쓸 때 `onNodesChange`/`onEdgesChange` 는 **한 쌍으로** 연결 (한쪽만 연결 = 그쪽 선택·삭제 무력화) ② "클릭하면 곧바로 파괴적 동작"인 UI 는 선택 경로의 결함을 가린다 — 클릭은 선택, 변경은 명시 버튼 원칙 ③ 합성 이벤트(dispatchEvent) 로는 React Flow 핸들러가 안 걸리므로 검증은 실제 마우스 클릭(path 위 좌표)으로

### [E-054] 배치 저장이 네트워크 장비 대장을 조용히 늘림 (180 → 295)

- **날짜:** 2026-07-26
- **증상:** 링크 에디터 "자동 정렬" 1회 실행 후 네트워크 장비 대장이 180 → 295 행으로 증가. 늘어난 117행은 IP·설명 없는 빈 행(`meta.canvas_pos` 만 존재)
- **원인:** 배치 저장 API 가 `INSERT … ON CONFLICT DO UPDATE` (시리얼 장비도 좌표를 저장할 수 있게 한 의도적 설계, spec §2.2). 개별 드래그 시엔 1~2행이라 눈에 띄지 않았으나 전체 정렬은 미등록 노드 전량을 대장에 등록 — **화면 배치라는 표시 목적 조작이 자산 대장을 바꾸는** 부수효과
- **해결:** 설계(UPSERT)는 유지하되 **조용하지 않게** — API 가 `created`(신규 등록 수, `RETURNING (xmax=0)`)를 반환하고, 자동 정렬 확인창에 "통신 정보가 없는 장비 N대는 배치 저장을 위해 대장에 등록됩니다" 사전 고지 + 완료 토스트·드래그 저장 토스트에 등록 수 표시. 검증 중 생성된 117행은 삭제해 178행으로 원복
- **재발 방지:** ① UPSERT 로 "부수적으로 마스터 행이 생기는" API 는 생성 건수를 반환하고 UI 가 고지 ② 일괄(bulk) 조작은 단건 조작과 부수효과의 **규모**가 다르므로 별도 검증 ③ 검증으로 만든 행은 검증 종료 시 카운트로 원복 확인
- **✅ 마감 (같은 날, P2-a):** 고지가 아니라 **원인 제거**로 전환 — Migration 0126 으로 표시 전용 테이블 `tb_canvas_layout(layer, node_key, x, y)` 신설, 배치 좌표를 대장에서 분리(178행 이관 + `meta - 'canvas_pos'`). 자동 정렬 295건 저장 후 대장 178행 불변·오염 0·새로고침 배치 유지 검증. `created` 고지 UI 는 제거. 교훈: **자산 대장에 표시 좌표를 넣지 않는다** (다이어그램 테이블인 `tb_flow_diagram_node` 는 좌표 보유가 정상 — 테이블의 성격으로 판단)

### [E-055] 기초정보에 등록한 시설이 계통도에 나타나지 않음 — 배치 대상이 관계 정본뿐

- **날짜:** 2026-07-27
- **증상:** 시설을 등록해도(시설정보 구축 화면 또는 등록 마법사) 계통도 캔버스·구축 트리에 보이지 않음. `relayout new_only` 는 "신규 배치 대상 없음"만 반환
...
### [E-059] 향후 전망 구간이 차트에서 뭉개져 보임 — category 축에 다른 간격 시리즈 연결

- **날짜:** 2026-07-29 (매방리 압력 트렌드 카드 — "데이터는 정상인데 그래프가 이상해 보임" 보고)
- **증상:** 전망(주황 점선)이 차트 오른쪽 끝에 좁게 뭉쳐 빠른 지글거림처럼 보임. 수치(전망 범위·도달 판정)는 전부 정상
- **원인:** ECharts x축이 **category(포인트당 등폭)** 인데, 관측(5분 버킷 ≈ 2,016점) 뒤에 전망(30분 간격 48점)을 그대로 이어붙임 — 24h 전망이 정상 폭(창의 1/8)의 1/6 로 압축돼 파형이 왜곡. 반대로 장기 창(버킷>30분)에선 전망이 과도하게 넓어지는 역왜곡
- **해결:** `comparison-overlay.ts` `resampleForecastToBucket` — 전망 시리즈·밴드를 **관측 버킷 간격으로 선형 재보간** 후 연결. category 축에서도 시간 비례 성립 (실측: 전망 폭 2.3% → 9.2% ≈ 이론값 플롯의 1/8)
- **재발 방지:** ① category 축에 간격이 다른 시리즈를 이어붙일 때는 반드시 간격을 맞춰 재보간 ② "수치는 맞는데 그래프 모양이 이상하다"는 축 스케일(등폭 vs 시간 비례) 왜곡 의심 신호 ③ 검증은 스크린샷 대신 SVG 좌표 실측(bbox 폭/전체 폭)이 정량적

### [E-060] "전체 배수지 유출유량 표" 채팅이 항상 0건 — 리팩토링 import 누락 NameError 가 "데이터 없음"으로 위장

- **날짜:** 2026-09-01
- **증상:** "전체 배수지의 유출 유량을 표로 보여줘"(FACILITY_CATALOG_TREND_TABLE)가 항상 "조회된 데이터가 없습니다". 분류·파라미터는 전부 정상
- **원인:** 시계열 헬퍼를 `shared/timeseries.py` 로 공용화하는 리팩토링 때 `sql_executor.py` import 목록에서 `query_chunks_agg`·`reaggregate` 2개 별칭이 누락. 이 인텐트만 그 둘을 쓰므로 여기서만 `NameError` — 핸들러 except 가 "조회 결과 없음"으로 삼켜 **정상적 빈 결과처럼 위장**됨
- **해결:** import 4종으로 확장. 재검증: 동일 질문 152행 반환 (AST 스캔으로 잔여 미정의 헬퍼 없음 확인)
- **재발 방지:** ① 모듈 분리 리팩토링 후에는 **호출부 전 파일 AST 미정의 이름 스캔** (grep 은 정의처만 보고 놓친다) ② "데이터 없음" 응답 경로에서 예외를 삼킬 때 최소 logger.error 는 남길 것 — 이번에도 로그에는 NameError 가 있었고 로그만 봤으면 즉시 잡혔다 ③ 채팅 스모크에 카탈로그 표 인텐트 케이스 추가 검토 (16개 대표군에 없어 회귀가 침묵)

### [E-061] 야간최소유량 트렌드에 "평소 대비 정상" 오배지 — 물리량이 다른 기준선과 비교

- **날짜:** 2026-09-01 (남산1 소블록 실사 — "이상해 보이는데 왜 정상이냐" 보고)
- **증상:** 채팅 야간최소유량 트렌드 카드가 편차 -68.1%인데 "평소 대비 정상". 실데이터는 8/8~18 원시값 32.30 고착(센서 동결) 후 급락 — 명백한 이상 구간
- **원인:** FACILITY_TREND 야간최소유량 fast-path 가 행을 **일별 NMF 집계**로 교체하는데, 이후 범용 트렌드 비교가 그대로 붙어 **원시 시간평균 기준선(38~91)** 과 비교 — 물리량이 달라 편차는 무의미하고 원시 유량의 큰 σ 때문에 z-score 가 우연히 '정상'. 한편 전용 감시(누수 CUSUM)는 같은 구간을 이미 "누수의심"으로 3회 검출(8/16~18) — 판정 체계가 이중으로 존재했고 잘못된 쪽이 화면에 노출된 것
- **해결:** fast-path 에서 `_nmf_series` 턴 파생 플래그(E-037 `_` 접두) → `_compute_comparison_map` 스킵. NMF 카드에는 배지 없음 — NMF 판정은 누수 CUSUM 전담. 일반 유량 트렌드 배지는 회귀 없음 확인
- **재발 방지:** ① 핸들러가 **행을 파생 집계로 교체**하면 하류 범용 후처리(비교·이상구간 등)가 여전히 유효한지 반드시 점검 ② 같은 신호에 판정 체계가 둘이면 화면에는 전담 체계 하나만 — 나머지는 스킵 ③ "편차가 큰데 정상" 조합은 기준선 물리량 불일치 의심 신호

### [E-062] 재기동 직후 "전체 이상 스캔" 1~2분 공백 — 인메모리 캐시 소실

- **날짜:** 2026-09-01
- **증상:** "전체 이상 스캔해줘"가 "전체 센서 점검 데이터를 준비 중입니다 (서버 시작 후 약 1~2분 소요)" 응답. 개발 중 파일 변경 리로드·재기동이 잦아 자주 노출 (당일 47회 리로드), 운영에서도 배포·재부팅 직후마다 발생
