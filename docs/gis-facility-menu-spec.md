...
### 5.3 권한·감사
- `tb_auth_menu` 권한 기반 접근. 멀티테넌시: `region` 격리. 전역 상태 금지.
- **감사 로그 (구현 완료 — Migration 0094, 1단계 설비 CRUD)**:
  `tb_audit_log`(actor·action·target_type·target_key·summary·detail·client_ip·created_at).
...
**시설 단위 고장 요약 추가 (2026-07-02):** 인스펙터에 최근 12개월 총 고장·진행중·
영향 설비·평균 조치시간 카드(`/monitoring/equipment-health/summary` 재사용). 실데이터
검증(행정 총5/진행4/영향1). 

**후속(미구현):** 시설 단위 월별 추세 차트, 대시보드 "교체 권고" KPI 딥링크,
**설치일자(commissioned_at) 데이터 입력**(고객 운영 데이터 — 조작 금지, 설비 CRUD
설치일 필드로 입력), CRUD **감사(audit) 로깅**(현재 미구현 — 별도 설계 필요).
