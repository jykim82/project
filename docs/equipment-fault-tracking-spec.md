...
`isFaultRecordIntent(text)` 조건 (`hooks/use-chat-submit.ts`):
1. `(고장|이상|교체|오류|파손|단락|정전)` 포함
2. `(배수지|가압장|감압|정수장|취수장|소블록|블록|댐)` 포함
3. **이상감지/분석 조회 제외** — `(스캔|감지|진단|점검|분석|예측|이상치|패턴|추이|통계|현황|이력)` 이
   포함되면 제외. 단 명시적 기록 표현 `(기록|저장|남겨|등록|발생|났|발견)` 이 있으면 유지.
4. **읽기 질의 제외** — `(상태|어때|얼마|몇|보여|알려|조회)` 이 있고 기록 표현이 없으면 제외.

1·2 충족 + 3·4 미제외 → `/chat/fault/draft` 경로.

> 3번 제외 규칙 배경: "행정 배수지 센서이상 스캔"(이상감지 스캔 인텐트, ANOMALY_SCAN_ALL)이
> "이상"+"배수지" 만으로 장애 기록 초안으로 오라우팅되던 문제 해결 (2026-07-12).
> 스캔·감지·분석 등은 **조회 인텐트**이므로 `/ask/stream` 으로 보내야 한다.
