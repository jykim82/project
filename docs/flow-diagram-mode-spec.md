...
**작성:** 2026-04-15 (초안), 2026-04-16 (B안 구현 완료 + 좌표 충돌 수정)
**상태:** B안 구현 완료
...
- **원인:** `seed_flow_diagram.py`의 `_place_subtree`에서 모든 자식(리프/브랜치)을 동일 로직으로 배치. 브랜치 자식의 서브트리 높이를 고려하지 않아 고정 Y 오프셋(`ROW_Y_GAP`)만 적용 → 다른 부모의 자식끼리 동일 (x,y) 충돌 13건
- **수정:** 리프/브랜치 자식 분리 배치. 리프는 가로 한 줄 (MAX_PER_ROW wrap), 브랜치는 서브트리 높이 반환값으로 Y 누적 추적. 수정 후 99노드 고유 좌표, 최소 간격 0.01도
- **커밋:** `slm@a72a88c`
