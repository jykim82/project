...
## 회의 통화 + 그룹 채팅 (2026-07-20)

- 3~4인 회의 통화 (0112): 방 생성→초대 벨→수락 합류, 풀메시 P2P (공유
  마이크+상대별 피어), 쌍별 SDP 사서함+하트비트, 참가자 바(상태 점)
- 그룹 채팅 (0113): 새 대화 다중 선택 (1명=DM/2명+=그룹), 제목 자동,
  기존 메시지·읽음·첨부·토스트 인프라 재사용, 비멤버 403
- 알람 팝업 3건: 해제 초록 표시 / "알람 제어" 명칭 / 이 알람 중지 프리필


---

## 업무 메모 달력 보기 (2026-07-20)
...
## 로드맵 C·D + UI 결정 반영 + E-039⑧ (2026-07-28)

**로드맵 C — 오탐 피드백 P1 (0132, alarm-label-feedback-spec)**
- 3분법 판정 real/false/check (기존 is_false_alarm 'N' 기본값은 미판정과
  구분 불가 — 오탐률 분모 부재). 정본 anomaly_label + 파생 동기화
- 이력 1클릭 판정 + "오탐 분석" 탭 (주별 추이·분류별·유보 배지 30건).
  백필 1,457건 (판정자 소급 금지). 가중치 조정은 P2 유보

**로드맵 D — 상황보고 1·2보 P1 (0133, incident-report-spec)**
- /reports/incident (M005-6) — 자동 채움(경보 접기·조치·전조 60분) 초안 →
  편집 → 확정. 후속보는 직전 보 이후 구간만. 세대수는 데이터 부재로 수동
- 보호 규칙 실검증: 확정 후 수정 불가·후속보 체인 삭제 보호·초안만 삭제

**폐기·UI 결정**
- **교대 인수인계 메뉴째 폐기** (0134 — 화면·API·메뉴·SHIFT_BOUNDARIES 제거,
  재제안 금지 memory). 점검 도래(0130)는 백엔드 잔존(표시처 없음)
- 임계 도달 예측 정확도 백테스트 (247건: 60분 전 중앙 ±20분·p90 ±56분) →
  ETA 구간 표시로 교정 (분 단위 = 거짓 정밀). Chronos 미채택 근거 §2.2
- 네트워크 토폴로지 계층형 우선(기본+버튼 순서) · 메모 달력 기본 ·
  보고서 제목 인라인 편집 · QR 라벨 제외 결정(재제안 금지)

**에러**
- E-039⑧ 채팅 트렌드 리드로우 재발 — ② 패턴이 PlotChart 잔존 (실측 202건
