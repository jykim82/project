...
### 완료 (2026-04-16 — 용수흐름도 다이어그램 모드 좌표 충돌 수정 [E-026])

- 증상: 줌 13에서 석문2 소블록, 행정1-2 감압설비 등 30+ 노드가 다른 노드와 동일 좌표에 배치 → 도형 겹침
- 원인: `seed_flow_diagram.py` `_place_subtree`에서 브랜치 자식 서브트리 높이 미반영, 고정 Y 오프셋만 사용 → 13건 좌표 충돌
- 수정: 리프/브랜치 분리 배치 + 서브트리 높이 반환값 Y 누적 추적. 99노드 모두 고유 좌표 확인
- 커밋: `slm@a72a88c`
