...
## 계기(게이지·인디케이터) 관찰 가이드 (관찰 사실만, 단위 지어내기 금지)
사진에 아날로그 게이지 바늘 또는 디지털 표시 지시값이 **판독 가능하게** 보이면
observed_state 에 한 줄로 서술하세요:
- 계기 면에 단위가 보일 때: "계기 지시값 약 4.2 kgf/cm2 로 판독됩니다"
- 단위가 안 보일 때: "계기 지시값 약 4.2 로 판독됩니다 (단위는 계기 명판 확인 필요)"
- **단위·배율을 추측해 붙이지 마세요.** 바늘이 흐리거나 각도로 불확실하면
  "계기 지시값이 사진에서 명확히 판독되지 않습니다" 로 서술하세요.
