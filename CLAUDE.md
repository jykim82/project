...
- `docs/kiosk-mode-spec.md` — 상황실 키오스크 모드 v1+P2 (P1 0141: `/kiosk` M003-12, 기존 화면 iframe 상주+opacity 순환, ?kiosk=1 크롬 제거, 경고 인터럽트 60s. **P2 0142 완료**: KIOSK_VIEWS 화면 목록·순서 커스텀(허용 5종, 인터럽트는 목록 무관 상주) + KIOSK_NIGHT_DIM 야간 밝기(자정 걸침 지원, 오버레이) + KIOSK_KPI_STRIP 요약 스트립(기존 API 조합, 새 API 없음). 설정 실패 시 P1 폴백)
- `docs/module-version-spec.md` — 모듈 버전·라이선스 관리 v1 (P1, Migration 0138 — tb_module_version 시드 8모듈 + tb_module_license(SKU 는 tb_comm_code 정본 실시간 조인·oss_notices). `/admin/system-modules` M100-17: 버전·live health(병렬 2s)·라이선스 배지 3종·OSS NOTICE(납품 고지 답변처). 업데이트 적용·롤백은 P2/P3 — 호스트 에이전트 필요, DB 롤백은 자동화 금지)
...
- `docs/trend-comparison-spec.md` — 트렌드 비교 지표 v1 (평소 대비 / 향후 전망 두 토글 + KPI 배지, anomaly_detector z-score 알람과 통일, NMF/CUSUM 흡수. **2026-07-29 확장 4건**: ① 수위 하한 판정 — 하강 추세는 실측 LL 임계로 "저수위 한계 도달" ② 유량 시간대 중앙값 프로파일(`hourly_profile`) — 계절 게이트 미달 시 Chronos 대신, 백테스트 81태그 76승·+29%, 이봉 신호에 실재값만·밴드 p10~p90 ③ 전망 렌더 재보간 [E-059] — category 축 시간 비례 ④ 진폭 압축 "참고" 톤 — 전망 범위 < 관측 진동 40% 시 low_confidence, CV 게이트가 놓치던 수위 진동 신호 커버)
