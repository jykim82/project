...
- `docs/kiosk-mode-spec.md` — 상황실 키오스크 모드 v1 (P1, Migration 0141 — `/kiosk` M003-12. 기존 3화면(GIS·계통도·경보) iframe 상주+opacity 순환(재로딩 없음), ?kiosk=1 크롬 제거, 하단 바(시계·도트·티커·전체화면), 경고 경보 인터럽트 60s(주의는 티커만 — 채터링 방지). 새 화면 안 만듦 — 기존 화면이 콘텐츠)
